...
"""
Code adapted from:
https://github.com/gwinndr/MusicTransformer-Pytorch/blob/master/model/rpr.py
...
class TransformerDecoderRPR(Module):
    ...
    def __init__(self, decoded_layer, num_layers, norm=None):
        super(TransformerDecoderRPR, self).__init__()
        self.layers = _get_clones(decoded_layer, num_layers)
        self.num_layers = num_layers
        self.norm = norm
...
class TransformerDecoderLayerRPR(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, er_len=None):
        ...
        self.self_attn = MultiheadAttentionRPR(d_model, nhead, dropout=dropout, er_len=er_len)
        self.multi_attn = MultiheadAttentionRPR(d_model, nhead, dropout=dropout, er_len=er_len)
        # Implementation of Feedforward model
        self.linear1 = Linear(d_model, dim_feedforward)
        self.dropout = Dropout(dropout)
        self.linear2 = Linear(dim_feedforward, d_model)
    ...
    def forward(self, tgt, memory, tgt_mask=None, memory_mask=None, tgt_key_padding_mask=None, memory_key_padding_mask=None):
        ...
        x = self.norm1(x)
        x = x + self.dropout2(self.multi_attn(x, memory, memory, attn_mask=memory_mask,
